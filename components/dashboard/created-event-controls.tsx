...
const CreatedEventControls = ({ eventId }: { eventId: bigint }) => {
  ...
  const submit = (name: "public" | "allowlist" | "airdrop") => {
  ...
  return (
    <Card>
      ...
      <CardContent className="flex flex-col gap-4">
        ...
        <Button type="button" disabled={frozen || pending} onClick={() => submit("public")} aria-busy={pending}>{event.isPublic ? "Close public mint" : "Open public mint"}</Button>
        <div className="flex flex-col gap-2"><label htmlFor="allowlist-root" className="text-sm font-medium">Allowlist root</label><Input id="allowlist-root" value={rootInput} onChange={(event) => setRootInput(event.target.value)} placeholder="0x followed by 64 hexadecimal characters" disabled={frozen || pending || event.allowlistRoot !== ZERO_ROOT} /><Button type="button" variant="outline" disabled={frozen || pending || event.allowlistRoot !== ZERO_ROOT} onClick={() => submit("allowlist")}>{event.allowlistRoot === ZERO_ROOT ? "Set allowlist root" : "Allowlist root already set"}</Button></div>
        <div className="flex flex-col gap-2"><label htmlFor="airdrop-recipients" className="text-sm font-medium">Creator airdrop recipients</label><textarea id="airdrop-recipients" value={recipientsInput} onChange={(event) => setRecipientsInput(event.target.value)} placeholder="One address per line" disabled={frozen || pending} className="min-h-24 rounded-md border border-input bg-transparent px-3 py-2 text-sm" /><Button type="button" variant="outline" disabled={frozen || pending} onClick={() => submit("airdrop")}>Airdrop to recipients</Button></div>
